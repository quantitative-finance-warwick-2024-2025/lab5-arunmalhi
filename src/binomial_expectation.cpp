...
double binomial_expectation_loop(double *spread, double p, unsigned int n) {
    std::vector<double> arr(n + 1, 0.0);    // Create array using a vector of size n+1

    // set up the base nodes
    for (unsigned int i = 0; i<n+1; ++i){
        arr[i] = spread[i];                 // For each element in the array, it is the spread (V(n,j) = spread(j))
    }
    ...
    for (int i = (int)n - 1; i >= 0; --i)
    {
        ...
        {
            arr[j] = (1.0 - p) * arr[j] + p * arr[j + 1];   // V(i,j) = (1-p)V(i+1,j) + pV(i+1,j+1)
        }
    }

    double answer = arr[0];         //V(0,0)
    return answer;
}
...
double binomial_expectation_recursion_h(
    ...
    return (1.0 - p) * binomial_expectation_recursion_h(i+1,j,spread,p,n) +
        p * binomial_expectation_recursion_h(i+1,j+1,spread,p,n);           //V(i,j) = (1-p)V(i+1,j) + pV(i+1,j+1)
}
...
void expected_returns(double S0,
    ...
    for (size_t i = 0; i<spread_points; ++i){
        ...
        out_returns[i] = S0*expected_returns(S0,number_of_periods,volatility,time_period,out_spread[i]);
    }
}
